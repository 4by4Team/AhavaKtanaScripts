...
function generateStickersPages(data, templateFolder, outputFolder) {
    for (var i = 0; i < data.length; i++) {
        ...
        // שם הקובץ כולל הכמות
        var baseName = "["+item.referenceNumber+"] "+item.name+ " $"+ item.itemName + "$ " + " (" + item.quantity + ").pdf";
        ...
        try {
            ...
            alert("❌ שגיאה בשמירת PDF עבור '" + baseName + "': " + e.message);
            doc.close(SaveOptions.DONOTSAVECHANGES);
            continue;
        }
        ...
        try {
            ...
            alert("⚠️ שגיאה בסגירת הקובץ עבור '" + baseName + "': " + e.message);
        }
    }
